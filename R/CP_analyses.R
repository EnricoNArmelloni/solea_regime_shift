#########R flow to analyze Change Point and regime shifts############
load("solea_tot.RData")
...
#change point 
cpt.mean(solea_tot$AbunRecruits)
cpt.meanvar(solea_tot$AbunRecruits)


cpt.mean(solea_tot$AbunAdults)
cpt.meanvar(solea_tot$AbunAdults)

cpt.mean(solea_tot$Abun_tot)
cpt.meanvar(solea_tot$Abun_tot)
...
p1 <- ggplot(data = sol,
             aes(bottom_t, Abun_tot)) + geom_point() + geom_line(aes(y=pred), col="#225ea8")+theme_bw()+ xlab("Sea Bottom Temperature (°C)")+
...
p <- ggplot(data = solea_tot, aes(x = runoff_ms, y = Abun_tot)) + 
  ...
  geom_smooth(data=solea_max,aes(x = runoff_ms, y = Abun_tot), method = "lm", se = FALSE, col="#00AFBB")+
  geom_smooth(data=solea_min,aes(x = runoff_ms, y = Abun_tot), method = "lm", se = FALSE, col="#FC4E07")+
...
pred <-predict(m1)
p1 <- ggplot(data = cbind(solea_tot, pred),
             aes(bottom_t, AbunAdults)) + geom_point() + geom_line(aes(y=pred), col="#41b6c4")+theme_bw()+ xlab("Sea Bottom Temperature (°C)")+
